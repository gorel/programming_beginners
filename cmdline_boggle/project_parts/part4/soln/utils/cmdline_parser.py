...
class CmdlineParser(object):
    def __init__(self):
        ...
        parser.add_argument("-f", "--scores_file", default=DEFAULT_HIGH_SCORES_FILENAME, help="filename storing high scores")
        parser.add_argument("--debug", action="store_true", help="Adds additional debug printing")

        subparsers = parser.add_subparsers(help="Mode in which to run the program")
        ...
        play_parser.add_argument("-0", "--no_high_scores", action="store_true", help="Ignore high scores for this game")
        play_parser.set_defaults(which="play")

        # Subparser: show high scores
        highscores_subparser = subparsers.add_parser("highscores", help="Show high scores for the game")
        highscores_subparser.set_defaults(which="highscores")
